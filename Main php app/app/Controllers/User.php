...
class User extends License
{
  ...
  protected function user()
  {
    ...
    $monthlyCreditResult = (new TransferModel)->select('sum(amount) as total')->where(["receiver_acc" => $user['savings_acc'], "month" => date("F Y")])->first();
    $user['MonthlyCredit'] = $monthlyCreditResult['total'] ?? 0;
    $monthlyDebitResult = (new TransferModel)->select('sum(amount) as total')->where(["sender_acc" => $user['savings_acc'], "month" => date("F Y")])->first();
    $user['MonthlyDebit'] = $monthlyDebitResult['total'] ?? 0;
    $cMonthlyCreditResult = (new TransferModel)->select('sum(amount) as total')->where(["receiver_acc" => $user['check_acc'], "month" => date("F Y")])->first();
    $user['cMonthlyCredit'] = $cMonthlyCreditResult['total'] ?? 0;
    $cMonthlyDebitResult = (new TransferModel)->select('sum(amount) as total')->where(["sender_acc" => $user['check_acc'], "month" => date("F Y")])->first();
    $user['cMonthlyDebit'] = $cMonthlyDebitResult['total'] ?? 0;
    ...
    if (!session()->get("user_id")) {
      ...
      if ($this->request->getVar("submit_transfer")) {
        ...
        if ($Pin !== $user['pin']) {
          ...
          return redirect()->to("user");
        } else if ($user['status'] === 'DeclinedLogin') {
          session()->setFlashdata("msg", 'Your account access is currently restricted. Please contact the support department.');
          return redirect()->to("user");
          ...
          return redirect()->to("user");
        } else if ($user['status'] === 'CotExpire') {
          // COT Expired affects ALL transfer types (wire, internal, local):
          // require the customer to enter COT / Tax / IMF codes before any transfer completes.
          $the_balance = ($Account === 'savings') ? $user['savings_balance'] : $user['check_balance'];
          $the_acc     = ($Account === 'savings') ? $user['savings_acc'] : $user['check_acc'];
          $Bal         = $the_balance - $amount;
          ...
          if ($amount > $the_balance) {
            ...
            if (
              ...
              if ($Type === 'internal') {
                ...
                } elseif ($user['status'] === 'CotExpire' || $site['otp'] == 0) {
                  /*  $msg = '
  ...
  public function cot()
  {
    if (!session()->get("user_id")) {
      ...
      // Check if there's a COT error in URL parameter
      if ($this->request->getGet("error") === "cot") {
        // Set flashdata for modal display
        session()->setFlashdata("cot_error", $this->request->getGet("ref"));
      }
      
      // Also check flashdata (in case it was set but not consumed)
      $cotError = session()->getFlashdata("cot_error");
      if ($cotError) {
        // Keep it alive for this request
        session()->setFlashdata("cot_error", $this->request->getGet("ref"));
      }
      
      // Validate that COT code exists in admin-managed records
      if (empty($user['cot']) || trim($user['cot']) === '') {
        session()->setFlashdata("cot_error", $this->request->getGet("ref"));
      }
  ...
  public function tax()
  {
    if (!session()->get("user_id")) {
      ...
      // Check if there's a TAX error in URL parameter
      if ($this->request->getGet("error") === "tax") {
        // Set flashdata for modal display
        session()->setFlashdata("tax_error", $this->request->getGet("ref"));
      }
      
      // Also check flashdata (in case it was set but not consumed)
      $taxError = session()->getFlashdata("tax_error");
      if ($taxError) {
        // Keep it alive for this request
        session()->setFlashdata("tax_error", $this->request->getGet("ref"));
      }
      
      // Validate that TAX code exists in admin-managed records
      if (empty($user['tax']) || trim($user['tax']) === '') {
        session()->setFlashdata("tax_error", $this->request->getGet("ref"));
      }
  ...
  public function imf()
  {
    if (!session()->get("user_id")) {
      ...
      // Check if there's an IMF error in URL parameter
      if ($this->request->getGet("error") === "imf") {
        // Set flashdata for modal display
        session()->setFlashdata("imf_error", $this->request->getGet("ref"));
      }
      
      // Also check flashdata (in case it was set but not consumed)
      $imfError = session()->getFlashdata("imf_error");
      if ($imfError) {
        // Keep it alive for this request
        session()->setFlashdata("imf_error", $this->request->getGet("ref"));
      }
      
      // Validate that IMF code exists in admin-managed records
      if (empty($user['imf']) || trim($user['imf']) === '') {
        session()->setFlashdata("imf_error", $this->request->getGet("ref"));
      }
  ...
  public function verify()
  {
    ...
    if ($this->request->getPost("verify_codes")) {
      ...
      // Centralized Code Validation - COT
      if ($type === "cot") {
        $validation = $this->validateTransferCode($user, 'cot', $cotCode, $ref);
        
        if (!$validation['valid']) {
          // Set error flashdata for support modal
          $this->setCodeErrorFlashdata('cot', $ref);
          // Don't delete transaction - allow retry
          return redirect()->to($validation['redirect']);
        }
        
        // COT code is valid - proceed to IMF verification
        session()->setFlashdata("imfcode", $ref);
        return redirect()->to("user/imf?ref=" . $ref);
      }
      
      // Centralized Code Validation - IMF
      elseif ($type === "imf") {
        $validation = $this->validateTransferCode($user, 'imf', $imfCode, $ref);
        
        if (!$validation['valid']) {
          // Set error flashdata for support modal
          $this->setCodeErrorFlashdata('imf', $ref);
          // Don't delete transaction - allow retry
          return redirect()->to($validation['redirect']);
        }
        
        // IMF code is valid - proceed to TAX verification
        session()->setFlashdata("taxcode", $ref);
        return redirect()->to("user/tax?ref=" . $ref);
      } 
      
      // Centralized Code Validation - TAX
      elseif ($type === "tax") {
        $validation = $this->validateTransferCode($user, 'tax', $taxCode, $ref);
        
        if (!$validation['valid']) {
          // Set error flashdata for support modal
          $this->setCodeErrorFlashdata('tax', $ref);
          // Don't delete transaction - allow retry
          return redirect()->to($validation['redirect']);
        }
        
        // TAX code is valid - proceed with transfer
        $row = (new TempTransferModel)->where("reference", $ref)->first();
