...
// ZipArchive - check if class exists before use
...
class Admin extends License
{
        ...
        if ($rules) {
          ...
          if ($uploadedFile->move('uploads/themes/', $filename)) {
            if (!class_exists('ZipArchive')) {
              session()->setFlashdata("msg", 'ZipArchive extension is not enabled. Please enable php_zip extension in php.ini');
              return redirect()->to("admin/settings");
            }
            $zip = new \ZipArchive();
  ...
  public function delete_records()
  {
    ...
    } else if ($this->request->getVar('delete_statement') && $this->request->getVar('id') && !$this->request->getPost('all') && session()->get("admin")) {
      // Delete individual statement/transaction (GET request with single ID, not bulk POST)
      $transferId = $this->request->getVar("id");
      $userId = $this->request->getVar("user_id");
      
      // Get user_id from transfer record if not provided in URL
      if (empty($userId) && $transferId) {
        $transfer = (new TransferModel)->where("id", $transferId)->first();
        if ($transfer && isset($transfer['user_id'])) {
          $userId = $transfer['user_id'];
        }
      }
      
      // Perform deletion - delete single transaction by ID
      $deleted = false;
      if ($transferId) {
        $deleted = (new TransferModel)->where("id", $transferId)->delete();
      }
      
      if ($deleted) {
        session()->setFlashdata("msg", 'Transaction Deleted Successfully!');
      } else {
        session()->setFlashdata("msg", 'Error Occurred. Please try again');
      }
      
      // Always redirect back to manage page with user ID to refresh and remain on page
      if ($userId) {
        return redirect()->to(base_url("admin/manage?id=" . $userId));
      } else {
