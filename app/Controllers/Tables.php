...
class Tables extends License
  ...
  public function index()
     ...
     ***************************/
      ...
      $Model->select("id, created_at,type,remarks,amount,reference,sender_acc,bank_name,receiver_acc,user_id")->where("receiver_acc", $main)->orwhere("sender_acc", $main);
      ...
      return DataTable::of($Model)
        ...
        ->add("action", function ($row) {
          // Get user_id from the row data (already selected in query)
          ...
          return '<a href="' . base_url('admin/receipt?ref=' . $row->reference . '&id=receipt') . '" class="btn btn-sm btn-success">RECEIPT</a>
                  ...
                  <a href="' . base_url('admin/delete_records?delete_statement=1&id=' . $row->id . '&user_id=' . $userId) . '" class="btn btn-sm btn-danger" onClick="return(confirm(\'Are you sure you want to delete this transaction?\'))">Delete</a>';
          }, "last")
        ->hide("sender_acc")
        ->hide("receiver_acc")
        ->hide("bank_name")
