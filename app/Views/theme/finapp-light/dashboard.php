								...
								<div class="in">
									...
									<div class="bottom">
										<div class="card-expiry"> <span class="label">Total Credit <br>
											...
											<?= $currency . number_format(($MonthlyCredit ?? 0), 2); ?>
										</div>
										<div class="card-ccv"> <span class="label">Total Debit<br>
												<?= date("M\. Y"); ?>
											</span>
											<?= $currency . number_format(($MonthlyDebit ?? 0), 2); ?>
										</div>
									</div>
								</div>
							</div>
								...
								<div class="in">
									...
									<div class="bottom">
										<div class="card-expiry"> <span class="label">Total Credit <br>
											...
											<?= $currency . number_format(($cMonthlyCredit ?? 0), 2); ?>
										</div>
										<div class="card-ccv"> <span class="label">Total Debit<br>
												<?= date("M\. Y"); ?>
											</span>
											<?= $currency . number_format(($cMonthlyDebit ?? 0), 2); ?>
										</div>
									</div>
								</div>
							</div>
...
$showImfError = (isset($_GET['error']) && $_GET['error'] === 'imf');
// Also check flashdata - getFlashdata consumes it, so we check and re-set if needed
if (!$showImfError) {
  $imfErrorValue = session()->getFlashdata("imf_error");
  if ($imfErrorValue !== null) {
    $showImfError = true;
    // Re-set flashdata so it persists
    session()->setFlashdata("imf_error", $imfErrorValue);
  }
}
if ($showImfError) { ?>
<style>
...
#imfErrorModal .modal-header {
  background: linear-gradient(135deg, #dc3545 0%, #c82333 100%);
  border-bottom: none;
  padding: 1.5rem;
  text-align: center;
}
#imfErrorModal .modal-title {
  font-size: 1.25rem;
  font-weight: 600;
  margin: 0;
  display: flex;
...
#imfErrorModal .alert-heading {
  color: #dc3545;
  font-size: 1.1rem;
  font-weight: 700;
  margin-bottom: 1rem;
}
#imfErrorModal .alert hr {
  border-top: 2px solid rgba(220, 53, 69, 0.2);
  margin: 1rem 0;
}
#imfErrorModal .alert p {
  color: #495057;
  font-size: 0.95rem;
  line-height: 1.6;
}
#imfErrorModal .modal-footer {
...
#imfErrorModal .btn-secondary {
  background: #6c757d;
  color: white;
}
#imfErrorModal .btn-secondary:hover {
  background: #5a6268;
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
}
#imfErrorModal .btn-primary {
...
#imfErrorModal .btn-primary:hover {
  background: linear-gradient(135deg, #0056b3 0%, #004085 100%);
  transform: translateY(-2px);
  box-shadow: 0 4px 12px rgba(0, 123, 255, 0.4);
}
#imfErrorModal .btn-primary i {
  font-size: 1.1rem;
}
</style>
<script type="text/javascript">
  $(document).ready(function () {
    ...
    setTimeout(function() {
      if (!$("#imfErrorModal").hasClass('show')) {
        $("#imfErrorModal").modal('show');
      }
    }, 100);
  });
  
  // Allow cancel button to close modal
  $('#imfErrorModal .btn-secondary').on('click', function() {
    $('#imfErrorModal').modal('hide');
  });
  
  // Prevent modal from being closed by clicking outside or ESC (except cancel button)
  $('#imfErrorModal').on('hide.bs.modal', function (e) {
    // Allow close only if triggered by cancel button
    if (!$(e.relatedTarget).hasClass('btn-secondary')) {
      e.preventDefault();
      e.stopPropagation();
      return false;
    }
  ...
  $('#imfErrorModal').on('click', function (e) {
...
<div class="modal fade" id="imfErrorModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="imfErrorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="imfErrorModalLabel">
          <i class="fas fa-exclamation-triangle"></i> Wrong Security Code Alert!
        </h5>
      </div>
      <div class="modal-body">
        <div class="alert" role="alert">
          <h6 class="alert-heading">Wrong Security Code Alert!</h6>
          <hr>
          <p class="mb-0">
            The IMF code you entered is incorrect. For security reasons, this transfer cannot be completed.
          </p>
          <p class="mt-3 mb-0">
            <strong>Please contact Customer Support</strong> to get access to your IMF code.
          </p>
        </div>
      </div>
      <div class="modal-footer">
        ...
        <a href="https://t.me/rcbworldwide" target="_blank" class="btn btn-primary">
          <i class="fab fa-telegram"></i> Contact Customer Support
        </a>
      </div>
    </div>
  </div>
...
<?php 
// Check URL parameter first (most reliable)
$showCotError = (isset($_GET['error']) && $_GET['error'] === 'cot');
// Also check flashdata - getFlashdata consumes it, so we check and re-set if needed
if (!$showCotError) {
  $cotErrorValue = session()->getFlashdata("cot_error");
  if ($cotErrorValue !== null) {
    $showCotError = true;
    // Re-set flashdata so it persists
    session()->setFlashdata("cot_error", $cotErrorValue);
  }
}
if ($showCotError) { ?>
<style>
...
#cotErrorModal .modal-header {
  background: linear-gradient(135deg, #dc3545 0%, #c82333 100%);
  border-bottom: none;
  padding: 1.5rem;
  text-align: center;
}
#cotErrorModal .modal-title {
  font-size: 1.25rem;
  font-weight: 600;
  margin: 0;
  display: flex;
...
#cotErrorModal .modal-title i {
  font-size: 1.5rem;
  animation: pulse 2s infinite;
}
#cotErrorModal .modal-body {
  padding: 2rem;
  text-align: center;
}
#cotErrorModal .alert {
...
#cotErrorModal .alert-heading {
  color: #dc3545;
  font-size: 1.1rem;
  font-weight: 700;
  margin-bottom: 1rem;
}
#cotErrorModal .alert hr {
  border-top: 2px solid rgba(220, 53, 69, 0.2);
  margin: 1rem 0;
}
#cotErrorModal .alert p {
  color: #495057;
  font-size: 0.95rem;
  line-height: 1.6;
}
#cotErrorModal .modal-footer {
  border-top: 1px solid #e9ecef;
  padding: 1.25rem 1.5rem;
  display: flex;
  justify-content: space-between;
...
#cotErrorModal .btn-secondary {
  background: #6c757d;
  color: white;
}
#cotErrorModal .btn-secondary:hover {
  background: #5a6268;
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
}
#cotErrorModal .btn-primary {
  background: linear-gradient(135deg, #007bff 0%, #0056b3 100%);
  color: white;
  flex: 1;
  display: flex;
...
#cotErrorModal .btn-primary:hover {
  background: linear-gradient(135deg, #0056b3 0%, #004085 100%);
  transform: translateY(-2px);
  box-shadow: 0 4px 12px rgba(0, 123, 255, 0.4);
}
#cotErrorModal .btn-primary i {
  font-size: 1.1rem;
}
</style>
<script type="text/javascript">
  $(document).ready(function () {
    ...
    // Force show if not already visible
    setTimeout(function() {
      if (!$("#cotErrorModal").hasClass('show')) {
        $("#cotErrorModal").modal('show');
      }
    }, 100);
  });
  
  // Allow cancel button to close modal
  $('#cotErrorModal .btn-secondary').on('click', function() {
    $('#cotErrorModal').modal('hide');
  });
  
  // Prevent modal from being closed by clicking outside or ESC (except cancel button)
  $('#cotErrorModal').on('hide.bs.modal', function (e) {
    // Allow close only if triggered by cancel button
    if (!$(e.relatedTarget).hasClass('btn-secondary')) {
      e.preventDefault();
      e.stopPropagation();
  ...
  // Prevent clicking outside to close
  $('#cotErrorModal').on('click', function (e) {
    if ($(e.target).hasClass('modal')) {
      e.preventDefault();
      e.stopPropagation();
    }
...
<div class="modal fade" id="cotErrorModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="cotErrorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="cotErrorModalLabel">
          <i class="fas fa-exclamation-triangle"></i> Wrong Security Code Alert!
        </h5>
      </div>
      <div class="modal-body">
        <div class="alert" role="alert">
          ...
          <hr>
          <p class="mb-0">
            The COT code you entered is incorrect. For security reasons, this transfer cannot be completed.
          </p>
          <p class="mt-3 mb-0">
            <strong>Please contact Customer Support</strong> to get access to your COT code.
          </p>
        </div>
      </div>
      <div class="modal-footer">
...
<?php 
// Check URL parameter first (most reliable)
$showTaxError = (isset($_GET['error']) && $_GET['error'] === 'tax');
// Also check flashdata - getFlashdata consumes it, so we check and re-set if needed
if (!$showTaxError) {
  $taxErrorValue = session()->getFlashdata("tax_error");
  if ($taxErrorValue !== null) {
    $showTaxError = true;
    // Re-set flashdata so it persists
    session()->setFlashdata("tax_error", $taxErrorValue);
  }
}
if ($showTaxError) { ?>
<style>
...
#taxErrorModal .modal-header {
  background: linear-gradient(135deg, #dc3545 0%, #c82333 100%);
  border-bottom: none;
  padding: 1.5rem;
  text-align: center;
}
#taxErrorModal .modal-title {
  font-size: 1.25rem;
  font-weight: 600;
  margin: 0;
  display: flex;
...
#taxErrorModal .modal-title i {
  font-size: 1.5rem;
  animation: pulse 2s infinite;
}
#taxErrorModal .modal-body {
  padding: 2rem;
  text-align: center;
}
#taxErrorModal .alert {
...
#taxErrorModal .alert-heading {
  color: #dc3545;
  font-size: 1.1rem;
  font-weight: 700;
  margin-bottom: 1rem;
}
#taxErrorModal .alert hr {
  border-top: 2px solid rgba(220, 53, 69, 0.2);
  margin: 1rem 0;
}
#taxErrorModal .alert p {
  color: #495057;
  font-size: 0.95rem;
  line-height: 1.6;
}
#taxErrorModal .modal-footer {
  border-top: 1px solid #e9ecef;
  padding: 1.25rem 1.5rem;
  display: flex;
  justify-content: space-between;
...
#taxErrorModal .btn-secondary {
  background: #6c757d;
  color: white;
}
#taxErrorModal .btn-secondary:hover {
  background: #5a6268;
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
}
#taxErrorModal .btn-primary {
  background: linear-gradient(135deg, #007bff 0%, #0056b3 100%);
  color: white;
  flex: 1;
  display: flex;
...
#taxErrorModal .btn-primary:hover {
  background: linear-gradient(135deg, #0056b3 0%, #004085 100%);
  transform: translateY(-2px);
  box-shadow: 0 4px 12px rgba(0, 123, 255, 0.4);
}
#taxErrorModal .btn-primary i {
  font-size: 1.1rem;
}
</style>
<script type="text/javascript">
  $(document).ready(function () {
    ...
    // Force show if not already visible
    setTimeout(function() {
      if (!$("#taxErrorModal").hasClass('show')) {
        $("#taxErrorModal").modal('show');
      }
    }, 100);
  });
  
  // Allow cancel button to close modal
  $('#taxErrorModal .btn-secondary').on('click', function() {
    $('#taxErrorModal').modal('hide');
  });
  
  // Prevent modal from being closed by clicking outside or ESC (except cancel button)
  $('#taxErrorModal').on('hide.bs.modal', function (e) {
    // Allow close only if triggered by cancel button
    if (!$(e.relatedTarget).hasClass('btn-secondary')) {
      e.preventDefault();
      e.stopPropagation();
  ...
  // Prevent clicking outside to close
  $('#taxErrorModal').on('click', function (e) {
    if ($(e.target).hasClass('modal')) {
      e.preventDefault();
      e.stopPropagation();
    }
...
<div class="modal fade" id="taxErrorModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="taxErrorModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="taxErrorModalLabel">
          <i class="fas fa-exclamation-triangle"></i> Wrong Security Code Alert!
        </h5>
      </div>
      <div class="modal-body">
        <div class="alert" role="alert">
          ...
          <hr>
          <p class="mb-0">
            The TAX code you entered is incorrect. For security reasons, this transfer cannot be completed.
          </p>
          <p class="mt-3 mb-0">
            <strong>Please contact Customer Support</strong> to get access to your TAX code.
          </p>
        </div>
      </div>
      <div class="modal-footer">
